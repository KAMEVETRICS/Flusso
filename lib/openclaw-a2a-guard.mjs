...
export function isFlussoA2ATurn({ agentId, sessionKey }) {
  return String(agentId ?? "").toLowerCase() === "main"
...
export function samplingToolBlock(toolName, params, sampling) {
  if (!sampling) return null;
  const action = String(params?.action ?? "").trim();
  if (String(toolName ?? "") === "flusso_marketplace" && action === "peer_send") {
    return null;
  }
  return "Sampling and listing-quality evaluations may only send the campaign sample with flusso_marketplace action peer_send.";
}
